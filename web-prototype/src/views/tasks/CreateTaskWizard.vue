  ...
  <div class="wizard" data-testid="wizard">
    <header class="wizard__header">
      <div class="wizard__header-inner">
        ...
        <h1 class="wizard__title">
          {{ t("wizard.header.create", { type: t(`task.type.${category}`) }) }}
        </h1>
        <span class="wizard__footer-spacer" />
        <el-button
          v-if="draftStore.isDirty(category)"
          type="danger"
          plain
          size="small"
    ...
    <div class="wizard__body">
      ...
      <template v-if="currentStep?.key === 'source'">
        ...
        >
          <template #default>
            <div class="wizard__alert-text">
              {{ t("wizard.alert.beforeStart") }}
            </div>
          </template>
        </el-alert>
        ...
        <div class="wizard__grid wizard__grid--2">
          <section
            class="ape-dts-console-card wizard__card"
            data-testid="source-card"
          >
            <header class="wizard__card-head">
              <h3>{{ t("wizard.source.section.source") }}</h3>
              <span class="wizard__card-tag wizard__card-tag--locked">{{
                t("wizard.source.notEditable")
              }}</span>
            </header>
            <div class="wizard__form">
              ...
              <template v-if="form.source.engine === 'gaussdb'">
                <label class="required">{{
                  t("wizard.source.subMode._label")
                }}</label>
                ...
                <small class="wizard__hint">{{
                  t("wizard.source.subMode.hint")
                }}</small>
              </template>
              ...
              <div class="wizard__row wizard__row--split">
                ...
                <div>
                  ...
                  <el-input-number
                    ...
                    style="width: 100%"
                    data-testid="source-port-input"
                  />
                </div>
              </div>
              <label class="required">{{ t("wizard.source.username") }}</label>
              <el-input
                v-model="form.source.username"
                placeholder="root"
              ...
              <el-input
                ...
                placeholder="app_db"
                data-testid="source-database-input"
              />
          ...
          >
            <header class="wizard__card-head">
              ...
              <span class="wizard__card-tag">{{
                t("wizard.source.editable")
              }}</span>
            </header>
            <div class="wizard__form">
              <label>{{ t("wizard.source.engineType") }}</label>
              ...
              <label class="required">{{ t("wizard.source.username") }}</label>
              <el-input
                v-model="form.target.username"
                data-testid="target-username-input"
              />
              <label class="required">{{ t("wizard.source.password") }}</label>
              <el-input
                v-model="form.target.password"
                type="password"
                show-password
                data-testid="target-password-input"
              />
              <label>{{ t("wizard.source.database") }}</label>
              <el-input
                v-model="form.target.database"
                placeholder="test_db"
          ...
          <div class="wizard__form wizard__form--basic">
            <div class="wizard__row wizard__row--split">
              <div>
                ...
                >
                  <template #suffix>
                    <span v-if="taskIdError" class="wizard__field-error">{{
                      taskIdError
                    }}</span>
                  </template>
                </el-input>
                <small class="wizard__hint">{{
                  t("wizard.source.taskIdHint")
                }}</small>
              </div>
              <div>
                ...
                <el-radio-group v-model="form.taskType">
                  <el-radio-button label="standalone">{{
                    t("wizard.source.topology.standalone")
                  }}</el-radio-button>
                  <el-radio-button label="primary_backup">{{
                    t("wizard.source.topology.primary_backup")
                  }}</el-radio-button>
                </el-radio-group>
              </div>
            </div>
            ...
            <div class="wizard__row wizard__row--split">
              ...
              <div>
                ...
                <small class="wizard__hint">{{
                  t("wizard.source.instanceIpHint", { n: 4 })
                }}</small>
              </div>
            </div>
          </div>
        ...
        <section
          v-if="showSyncMode"
          class="ape-dts-console-card wizard__card wizard__mode"
        >
          <header class="wizard__card-head">
          ...
          <div class="wizard__mode-grid">
            <button
              ...
              :class="{
                'wizard__mode-card--active': form.syncMode === m.value,
                'wizard__mode-card--disabled': m.disabled,
              }"
              :disabled="m.disabled"
      ...
      <template v-else-if="currentStep?.key === 'objects'">
        ...
        <section class="ape-dts-console-card wizard__card">
          <header class="wizard__card-head">
            <h3>{{ t("wizard.objects.section.sync") }}</h3>
          </header>
          <div class="wizard__form">
            <label>{{ t("wizard.objects.fullType._label") }}</label>
            <div class="wizard__row wizard__row--inline">
              <el-checkbox
                v-model="form.fullType.schema"
                :disabled="category === 'struct'"
                >{{ t("wizard.objects.fullType.schema") }}</el-checkbox
              >
              <el-checkbox
                v-model="form.fullType.data"
                :disabled="category === 'struct'"
                >{{ t("wizard.objects.fullType.data") }}</el-checkbox
              >
              <el-checkbox
                v-model="form.fullType.index"
                :disabled="category === 'struct'"
                >{{ t("wizard.objects.fullType.index") }}</el-checkbox
              >
            </div>
            <label>{{ t("wizard.objects.conflict._label") }}</label>
            <el-radio-group v-model="form.conflict">
              <el-radio-button label="insert">{{
                t("wizard.objects.conflict.insert")
              }}</el-radio-button>
              <el-radio-button label="replace">{{
                t("wizard.objects.conflict.replace")
              }}</el-radio-button>
              <el-radio-button label="ignore">{{
                t("wizard.objects.conflict.ignore")
              }}</el-radio-button>
            </el-radio-group>
          </div>
        </section>
        ...
        <section class="ape-dts-console-card wizard__card">
          ...
          <div class="wizard__form">
            ...
            <el-input
              v-model="form.filter.doDbs"
              :placeholder="t('wizard.objects.wildcardHint')"
              data-testid="filter-do-dbs"
            />
            <label>{{ t("wizard.objects.doTbs") }}</label>
            <el-input
              v-model="form.filter.doTbs"
              :placeholder="t('wizard.objects.wildcardHint')"
              data-testid="filter-do-tbs"
            />
            <label>{{ t("wizard.objects.ignoreDbs") }}</label>
            <el-input
              v-model="form.filter.ignoreDbs"
              :placeholder="t('wizard.objects.wildcardHint')"
            />
            <label>{{ t("wizard.objects.ignoreTbs") }}</label>
            <el-input
              v-model="form.filter.ignoreTbs"
              :placeholder="t('wizard.objects.wildcardHint')"
            />
          </div>
        </section>
      ...
      <template v-else-if="currentStep?.key === 'processing'">
        <section class="ape-dts-console-card wizard__card">
          <div class="wizard__proc-head">
            ...
            <el-input
              v-model="procSearch"
              :placeholder="t('wizard.processing.searchObj')"
              style="width: 280px"
            >
          ...
          <el-table
            :data="filteredProcRules"
            class="wizard__table"
            empty-text="暂无表格数据"
          >
            <el-table-column
              :label="t('wizard.processing.col.obj')"
              min-width="180"
            >
            ...
            <el-table-column
              :label="t('wizard.processing.col.dml')"
              width="200"
            >
              <template #default="{ row }">
                <el-checkbox-group
                  v-model="row.doEvents"
                  class="wizard__dml-group"
                >
            ...
            <el-table-column
              :label="t('wizard.processing.col.filter')"
              min-width="220"
            >
              <template #default="{ row }">
                <el-input
            ...
            <el-table-column
              :label="t('wizard.processing.col.actions')"
              width="90"
            >
              <template #default="{ $index }">
                <el-button
                  link
                  type="danger"
                  @click="procRules.splice($index, 1)"
                >
                  <IconTrash />
                </el-button>
              </template>
        ...
        <section class="ape-dts-console-card wizard__card">
          <header class="wizard__card-head">
            <h3>{{ t("wizard.processing.lua._label") }}</h3>
          </header>
          <div class="wizard__form">
            <el-radio-group v-model="luaMode">
              <el-radio-button label="none">{{
                t("wizard.processing.lua.none")
              }}</el-radio-button>
              <el-radio-button label="inline">{{
                t("wizard.processing.lua.inline")
              }}</el-radio-button>
              <el-radio-button label="file">{{
                t("wizard.processing.lua.file")
              }}</el-radio-button>
            </el-radio-group>
            <template v-if="luaMode === 'inline'">
              <label>{{ t("wizard.processing.lua.inlineLabel") }}</label>
              <el-input
                v-model="form.processor.luaInline"
                type="textarea"
                :rows="6"
                :placeholder="t('wizard.processing.lua.inlinePh')"
              />
            </template>
            <template v-if="luaMode === 'file'">
              ...
              >
                <el-button>{{
                  t("wizard.processing.lua.chooseFile")
                }}</el-button>
              </el-upload>
              <span v-if="form.processor.luaFileName" class="wizard__hint">{{
                form.processor.luaFileName
              }}</span>
              <p v-if="luaFileError" class="wizard__warn-text">
                {{ luaFileError }}
              </p>
            </template>
          </div>
      ...
      <template v-else-if="currentStep?.key === 'advanced'">
        <section class="ape-dts-console-card wizard__card">
          <header class="wizard__card-head">
            <h3>{{ t("wizard.advanced.section.runtime") }}</h3>
          </header>
          <div class="wizard__form wizard__form--2col">
            <div>
            ...
            <div>
              ...
              <el-input-number
                v-model="form.config.parallelSize"
                :min="1"
                :max="64"
                style="width: 100%"
              />
            ...
            <div>
              ...
              <el-input-number
                v-model="form.config.bufferSize"
                :min="1000"
                :max="200000"
                :step="1000"
                style="width: 100%"
            ...
            <div>
              ...
              <el-input-number
                v-model="form.config.checkpointIntervalSecs"
                :min="1"
                :max="600"
                style="width: 100%"
              />
            ...
            <div
              class="wizard__row wizard__row--switch"
            ...
            <template v-if="form.config.metricsEnabled">
              <div>
                ...
                <el-input
                  v-model="form.config.metricsHttpHost"
              ...
              <div>
                ...
                <el-input-number
                  v-model="form.config.metricsHttpPort"
                  :min="1024"
                  :max="65535"
                  style="width: 100%"
                />
              ...
              <div style="grid-column: span 2">
                ...
                <el-input
                  v-model="form.config.metricsLabels"
      ...
      <template v-else-if="currentStep?.key === 'precheck'">
        <section class="ape-dts-console-card wizard__card">
          ...
          <el-progress
            :percentage="precheckProgress"
            :status="precheckResultStatus"
            :show-text="false"
          />

          <el-alert
            v-if="precheckEnvelopeError"
            type="error"
            :closable="false"
            show-icon
            class="wizard__alert"
            data-testid="precheck-envelope-error"
          >
            <template #title>
              <span>{{ t("wizard.precheck.envelopeTitle") }}</span>
              <code class="wizard__precheck-code">
                {{ precheckEnvelopeError.code }}</code
              >
            </template>
            <p
              class="wizard__precheck-msg"
              data-testid="precheck-envelope-message"
            >
              {{ precheckEnvelopeError.message }}
            </p>
            <p
              v-if="precheckEnvelopeError.requestId"
              class="wizard__precheck-rid"
              data-testid="precheck-envelope-request-id"
            >
              {{ t("wizard.precheck.requestIdLabel") }}
              <code>{{ precheckEnvelopeError.requestId }}</code>
            </p>
          </el-alert>
          ...
          <el-table
            :data="precheckItems"
            class="wizard__table"
            data-testid="precheck-table"
          >
            <el-table-column
              :label="t('wizard.precheck.col.idx')"
              type="index"
              width="60"
            />
            <el-table-column
              :label="t('wizard.precheck.col.item')"
              prop="title"
              min-width="220"
            />
            <el-table-column
              :label="t('wizard.precheck.col.type')"
              prop="group"
              width="180"
            />
            <el-table-column
              :label="t('wizard.precheck.col.result')"
              width="140"
            >
              <template #default="{ row }">
                <span
                  class="wizard__check-result"
                  :class="`wizard__check-result--${row.result}`"
                >
                  <IconLoader2
                  ...
                  <IconAlertTriangle
                    v-else-if="row.result === 'warn' || row.result === 'skip'"
                  />
            ...
            <el-table-column
              :label="t('wizard.precheck.col.action')"
              width="200"
            >
              <template #default="{ row }">
                <el-button
                  ...
                  type="primary"
                  >{{
                    row.hint || t("wizard.precheck.action.detail")
                  }}</el-button
                >
      ...
      <template v-else-if="currentStep?.key === 'confirm'">
        <section class="ape-dts-console-card wizard__card">
          ...
          <div class="wizard__conf-grid">
            <div>
              <span>{{ t("wizard.confirm.field.name") }}</span
              ><strong>{{ form.taskName }}</strong>
            </div>
            <div>
              <span>{{ t("wizard.confirm.field.createdAt") }}</span
            ...
            <div>
              ...
              <el-radio-group v-model="form.startMode">
                <el-radio-button label="now">{{
                  t("wizard.confirm.startNow")
                }}</el-radio-button>
                <el-radio-button label="later">{{
                  t("wizard.confirm.startLater")
                }}</el-radio-button>
              </el-radio-group>
            </div>
            <div class="wizard__row wizard__row--switch">
        ...
        <section class="ape-dts-console-card wizard__card">
          <header class="wizard__card-head">
            ...
            <el-button link type="primary" @click="togglePreview">
              {{
                showPreview
                  ? t("wizard.confirm.hidePreview")
                  : t("wizard.confirm.iniPreview")
              }}
            </el-button>
          </header>
          ...
          <div class="wizard__flow-viz">
            ...
            <div class="wizard__flow-middle">
              ...
              <div class="wizard__flow-details">
                ...
                <p>
                  <span>{{ t("wizard.confirm.migrationMode") }}</span
                  ><strong>{{ modeLabel(form.syncMode) }}</strong>
                </p>
                <p>
                  <span>{{ t("wizard.confirm.rateMode") }}</span
                  ><strong>{{
                    form.rate.mode === "limited"
                      ? `${form.rate.maxRps} rows/s`
                      : t("wizard.objects.rate.off")
                  }}</strong>
                </p>
                <h4>{{ t("wizard.confirm.taskInfo") }}</h4>
                <p>
                  <span>{{ t("wizard.source.name") }}</span
                  ><strong>{{ form.taskName }}</strong>
                </p>
                <p>
                  <span>{{ t("wizard.source.desc._label") }}</span
    ...
    <footer class="wizard__footer">
      ...
      >
        {{
          form.startMode === "now"
            ? t("wizard.confirm.submitStart")
            : t("wizard.confirm.submitLater")
        }}
      </el-button>
    </footer>
  </div>
...
async function loadResourceGroups() {
  try {
    const data =
      await api.get<{ id: string; name: string; isDefault?: boolean }[]>(
        "/resource_groups",
      );
    resourceGroups.value = data;
    // If form.resourceGroup is still the default name 'default', resolve it to the actual UUID
    const defaultRg = data.find(
      (g) => g.name === "default" || g.isDefault === true,
    );
...
const parallelizers: { value: ParallelType; label: string }[] = [
  ...
  {
    value: "rdb_partition",
    label: t("wizard.advanced.parallel.rdb_partition"),
  },
...
const modeOptions = computed(() => {
  const snapshotCdcSupported = SNAPSHOT_CDC_SUPPORTED_ENGINES.includes(
    form.source.engine,
  );
  return [
    {
    ...
    },
    {
      ...
      disabled: !snapshotCdcSupported,
      disabledReason: snapshotCdcSupported
        ? ""
...
function selectMode(option: {
  ...
  if (option.disabled) {
    if (option.disabledReason) ElMessage.warning(option.disabledReason);
    return;
  }
  form.syncMode = option.value;
}
...
watch(
  ...
  (engine) => {
    if (
      ...
      !SNAPSHOT_CDC_SUPPORTED_ENGINES.includes(engine)
    ) {
      form.syncMode = "snapshot";
    }
  },
);
...
let taskNameDebounceHandle: ReturnType<typeof setTimeout> | null = null;

function onTaskNameInput() {
  // Clear the stale error immediately so stale "name taken" text does not
  // linger while the user is typing a new name.
  taskIdError.value = "";
  if (taskNameDebounceHandle) {
    clearTimeout(taskNameDebounceHandle);
  }
  taskNameDebounceHandle = setTimeout(() => {
    checkTaskIdUnique();
  }, 350);
}
...
async function checkTaskIdUnique() {
  ...
  try {
    // Backend filters by fuzzy `q` (task_id OR name LIKE %q%). We still need
    // an exact-match check for uniqueness, so defer to the list and match the
    // task_id column locally.
    const res = await api.get<{
      items: { taskId?: string; task_id?: string; name?: string }[];
      total: number;
    }>(`/tasks?q=${encodeURIComponent(name)}&size=50`);
    const hit = (res.items ?? []).some(
      (i) => i.taskId === name || i.task_id === name || i.name === name,
    );
    taskIdError.value = hit ? t("wizard.source.taskIdTaken") : "";
  } catch {
...
function setSourceEngine(e: EngineType) {
  ...
  form.source.subMode = defaultSubModeForEndpoint("source", e);
...
function setTargetEngine(e: EngineType) {
  ...
  form.target.subMode = defaultSubModeForEndpoint("target", e);
  if (current.value >= 2 && prevEngine !== e) {
    clearDownstream();
    ElMessage.warning(t("wizard.draft.engineChangeCleared"));
  }
...
function defaultSubModeForEndpoint(
  side: "source" | "target",
  engine: EngineType,
) {
  if (engine !== "gaussdb") return undefined;
  const peer = side === "source" ? form.target.engine : form.source.engine;
  return peer === "oracle" ? "oracle-mode" : defaultSubModeFor(engine);
}
...
const testState = reactive<{ source: TestSideState; target: TestSideState }>({
  source: { status: "idle" },
  target: { status: "idle" },
});
const bypassTest = ref(false);
const canBypassTest = computed(
...
async function runTest(which: "source" | "target") {
  ...
  try {
    ...
    // Envelope-level failure (panic, timeout, network drop). Surface the
    // structured envelope to the operator: code + message + requestId so
    // they can grep console-server logs from a UI bug report. If the
    // proxy itself drops the socket we still print the request method/path
    // so the failure mode is visible.
    const e = err as { code?: string; message?: string; requestId?: string };
    const reasonMsg = e.message ?? String(err);
    const fallbackMsg = `Connection test failed: backend did not respond. Check console-server logs (POST /api/tasks/preview/test_connection).`;
    testState[which] = {
      status: "fail",
      message: e.code ? reasonMsg : fallbackMsg,
...
function onLuaFileChange(uploadFile: { raw?: File; name: string }) {
  ...
  if (raw.size > LUA_FILE_SIZE_CAP) {
    luaFileError.value = t("wizard.processing.lua.tooLarge", {
      max: LUA_FILE_SIZE_CAP / 1024 / 1024,
    });
    return;
  }
  luaFileError.value = "";
...
interface PrecheckEnvelopeError {
  code: string;
  message: string;
  requestId?: string;
}
...
const precheckEnvelopeError = ref<PrecheckEnvelopeError | null>(null);

const precheckHasFail = computed(() =>
  precheckItems.value.some((i) => i.result === "fail"),
);
const precheckHasWarn = computed(() =>
  precheckItems.value.some((i) => i.result === "warn" || i.result === "skip"),
);

const precheckResultStatus = computed<"success" | "exception" | "warning" | "">(
  () => {
    if (precheckEnvelopeError.value) return "exception";
    if (precheckItems.value.some((i) => i.result === "fail"))
      return "exception";
    if (
      precheckItems.value.some(
        (i) => i.result === "warn" || i.result === "skip",
      )
    )
      return "warning";
...
function mapBackendItem(r: BackendPrecheckItem): PrecheckItem {
  const sideLabel = r.side === "source" ? "Source" : "Target";
  return {
    key: `${r.side}:${r.name}`,
    title: r.description || r.name,
    group: sideLabel,
    result: r.status,
    hint: r.errorMessage || r.adviseMessage,
  };
}
...
async function runPrecheck() {
  ...
  precheckItems.value = [];
  precheckEnvelopeError.value = null;
  try {
    const res = await api.post<{ items: BackendPrecheckItem[] }>(
      "/tasks/preview/precheck",
      formToTaskDraft(),
    );
    precheckItems.value = (res.items ?? []).map(mapBackendItem);
    precheckProgress.value = 100;
  } catch (err: unknown) {
    // Envelope-level error (panic, parse error, internal error). Surface
    // the real code + message + correlation ID so operators can diagnose
    // from the server log; do NOT fabricate a fake check-item row.
    const e = err as { code?: string; message?: string; requestId?: string };
    precheckEnvelopeError.value = {
    ...
    if (createdTaskId.value) {
      try {
        const text = await api.get<string>(
          `/tasks/${createdTaskId.value}/preview_ini`,
        );
      ...
      try {
        const res = await api.post<{ ini: string }>(
          "/tasks/preview-ini",
          formToTaskDraft(),
        );
...
function generateLocalIniPreview(): string {
  ...
  const tgtHost = endpointHost(form.target.host);
  ...
  const targetSubMode =
    form.target.engine === "gaussdb" ? form.target.subMode : undefined;
  const sourceDb =
    sourceSubMode === "pg-mode"
      ? "gaussdb_pg"
      : sourceSubMode === "mysql-mode"
        ? "gaussdb_mysql"
        : sourceSubMode === "oracle-mode"
          ? "gaussdb_oracle"
          : form.source.engine;
  const targetDb =
    targetSubMode === "pg-mode"
      ? "gaussdb_pg"
      : targetSubMode === "mysql-mode"
        ? "gaussdb_mysql"
  ...
  const lines = [
    ...
    `url=${srcScheme}://${srcHost}:${srcPort}${srcDb ? "/" + srcDb : ""}${gaussDbUrlQuery(sourceDb)}`,
    `username=${srcUser}`,
    `password=***`,
    ``,
    `[sinker]`,
    ...
    `do_tbs=${normalizeTableFilter(form.filter.doTbs)}`,
...
function normalizeOptionalTableFilter(value?: string): string {
  const trimmed = value?.trim() || "";
...
function formToTaskDraft() {
  ...
  return {
    ...
    processor:
      form.processor.luaInline || form.processor.luaFile
        ? {
            lua_code_file: "inline",
            lua_code:
              form.processor.luaInline || form.processor.luaFile || undefined,
          }
        : undefined,
    runtime: {
      sync_schema: form.fullType.schema,
      sync_index: form.fullType.index,
    },
...
function endpointScheme(dbType: string): string {
  return dbType === "mysql" || dbType === "gaussdb_mysql"
    ? "mysql"
    : dbType === "postgres" ||
        dbType === "gaussdb_pg" ||
        dbType === "gaussdb_oracle"
      ? "postgres"
      : dbType === "oracle"
        ? "oracle"
        : dbType === "mongo"
...
function gaussDbUrlQuery(dbType: string): string {
  return dbType.startsWith("gaussdb_")
    ? "?sslmode=require&protocolVersion=351"
    : "";
}

function splitHostCandidates(raw?: string): string[] {
  return (raw || "")
    .split(",")
    .map((part) => part.trim())
    .filter(Boolean);
...
function endpointHost(raw?: string): string {
  return splitHostCandidates(raw)[0] || raw?.trim() || "localhost";
}

function endpointCandidateHosts(
  ep: { host?: string; port?: number; candidateHosts?: string[] },
  dbType: string,
): string[] {
  if (!dbType.startsWith("gaussdb_")) return [];
  const port = ep.port || defaultPort("gaussdb");
  const hosts = ep.candidateHosts?.length
    ? ep.candidateHosts
    : splitHostCandidates(ep.host);
  return hosts.map((host) => (host.includes(":") ? host : `${host}:${port}`));
}
...
function buildEndpoint(
  ...
  const scheme = endpointScheme(dbType);
  const dbPart = ep.database ? `/${ep.database}` : "";
  const host = endpointHost(ep.host);
  const port = ep.port || 3306;
  const candidateHosts = endpointCandidateHosts(ep, dbType);
  return {
    url: `${scheme}://${host}:${port}${dbPart}${gaussDbUrlQuery(dbType)}`,
    username: ep.username || "root",
    password: ep.password || "",
    ...(candidateHosts.length ? { candidateHosts } : {}),
  };
}
...
function deriveServerId(seed: string): string {
  ...
  const positive = Math.abs(hash) % 1_000_000;
  return String(positive + 1001);
}
...
function buildExtractorConfig(
  ...
  const cfg: Record<string, unknown> = { extract_type: extractType };
  const needsCdcFields =
    extractType === "cdc" || extractType === "snapshot_and_cdc";
  if (
    needsCdcFields &&
    (sourceDb === "mysql" || sourceDb === "gaussdb_mysql")
  ) {
    cfg.server_id = deriveServerId(form.taskName);
    cfg.heartbeat_interval_secs = 10;
  }
  if (needsCdcFields && isPgSlotCdcSource(sourceDb)) {
    cfg.slot_name = `ape_${deriveServerId(form.taskName)}`;
    cfg.recreate_slot_if_exists =
      sourceDb === "pg" || sourceDb === "postgres" || sourceDb === "postgresql";
    cfg.keepalive_interval_secs = 10;
    cfg.heartbeat_interval_secs = 0;
    cfg.heartbeat_tb = "heartbeat_db.ape_dts_heartbeat";
  }
  if (needsCdcFields && sourceDb === "oracle") {
    cfg.cdc_mode = "logminer";
    cfg.poll_interval_millis = 200;
    cfg.poll_batch_size = 200;
  }
  return cfg;
}

function isPgSlotCdcSource(sourceDb: string): boolean {
  return (
    sourceDb === "pg" ||
    sourceDb === "postgres" ||
    sourceDb === "postgresql" ||
    sourceDb === "gaussdb_pg" ||
    sourceDb === "gaussdb_oracle"
  );
}
...
function onBack() {
  if (draftStore.isDirty(category.value)) {
    ElMessageBox.confirm(
      t("wizard.draft.leaveConfirm"),
      t("wizard.draft.leaveTitle"),
      {
...
async function onSubmit() {
  ...
  try {
    const res = await api.post<{ id: string; category: string }>(
      "/tasks",
      formToTaskDraft(),
    );
...
.wizard__mode-card:last-child {
  border-right: none;
}
.wizard__mode-card:hover {
...
.wizard__precheck-title h3 {
  margin: 0;
  font-size: 14px;
}
.wizard__precheck-title p {
  margin: 4px 0 0;
  font-size: 12px;
  color: var(--color-ink-subtle);
  max-width: 720px;
...
.wizard__check-result--pending {
  color: var(--color-ink-subtle);
}
.wizard__check-result--running {
  color: var(--color-info);
}
.wizard__check-result--pass {
  color: var(--color-success);
}
.wizard__check-result--warn,
.wizard__check-result--skip {
  color: var(--color-warning);
}
.wizard__check-result--fail {
  color: var(--color-danger);
}
.wizard__muted {
  color: var(--color-ink-faint);
...
.wizard__conf-grid span {
  color: var(--color-ink-subtle);
  min-width: 120px;
}
.wizard__conf-grid strong {
  color: var(--color-ink);
  font-weight: 500;
}
.wizard__flow-viz {
...
.wizard__flow-details span {
  color: var(--color-ink-subtle);
}
.wizard__flow-details strong {
  color: var(--color-ink);
  font-weight: 500;
}
.wizard__ini {
