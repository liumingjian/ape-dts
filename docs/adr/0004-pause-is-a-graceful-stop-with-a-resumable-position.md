...
- **Stopping a `paused` Run discards it** (`stop_method="discarded"`) without signalling anything — its pid may since have been recycled. A `pausing` Run is stoppable too: a pause whose engine will not converge must have a way out, or `is_active` freezes the task forever.
- **`config_file` in `[resumer]` stays empty.** The engine reads it as a *second position log*, line by line through `Position::from_log`; handing it the task INI would log every line as an invalid position and spill connection strings into the run's log directory. The position comes from `log_dir/position.log`, whose presence is checked before the resume starts.
...
- A console restart *inside* the drain window forfeits that pause: a `pausing` Run whose engine has already exited is reconciled as `failed`/`orphaned`, because the exit code — the only evidence that the position landed — died with the supervisor. This is the conservative side of the same rule ("never write `paused` without the exit code"); the position log survives on disk, only the resume affordance is lost. Recovering it would mean judging the position log's own contents, which belongs with the resume-position precheck rather than here.
