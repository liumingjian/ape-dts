...
5. `PG ↔ GaussDBOracle`（sync：snapshot/struct/check/cdc basic）
6. `Oracle ↔ GaussDBOracle`（bootstrap snapshot）
...
| Quick | `Oracle -> GaussDBOracle` struct basic | `cargo test -p dt-tests --test integration_test -- oracle_to_gaussdb_oracle::struct_tests::test::struct_basic_test --nocapture` | Oracle XE + GaussDBOracle | Oracle 源端对象同步（bootstrap） |
| Quick | `Oracle -> GaussDBOracle` check basic | `cargo test -p dt-tests --test integration_test -- oracle_to_gaussdb_oracle::check_tests::test::check_basic_test --nocapture` | Oracle XE + GaussDBOracle | Oracle 源端对账（bootstrap） |
| Quick | `Oracle -> GaussDBOracle` CDC basic | `cargo test -p dt-tests --test integration_test -- oracle_to_gaussdb_oracle::cdc_tests::test::cdc_basic_test --nocapture` | Oracle XE + GaussDBOracle | Oracle 源端增量（bootstrap logminer） |
| Quick | `Oracle -> GaussDBOracle` precheck basic | `cargo test -p dt-tests --test integration_test -- oracle_to_gaussdb_oracle::precheck_tests::test::struct_supported_basic_test --nocapture` | Oracle XE + GaussDBOracle | 预检查主路径（bootstrap） |
...
| Quick | `GaussDBOracle -> Oracle` struct basic | `cargo test -p dt-tests --test integration_test -- gaussdb_oracle_to_oracle::struct_tests::test::struct_basic_test --nocapture` | GaussDBOracle + Oracle XE | Oracle 目标端对象同步（bootstrap） |
| Quick | `GaussDBOracle -> Oracle` check basic | `cargo test -p dt-tests --test integration_test -- gaussdb_oracle_to_oracle::check_tests::test::check_basic_test --nocapture` | GaussDBOracle + Oracle XE | Oracle 目标端对账（bootstrap） |
...
| Quick | `GaussDBOracle -> Oracle` precheck basic | `cargo test -p dt-tests --test integration_test -- gaussdb_oracle_to_oracle::precheck_tests::test::struct_supported_basic_test --nocapture` | GaussDBOracle + Oracle XE | 预检查主路径（bootstrap） |
| Quick | `Oracle <-> GaussDBOracle` bootstrap batch（script） | `bash scripts/e2e/oracle_gaussdboracle_bootstrap.sh` | Oracle XE + GaussDBOracle | 一键回归（snapshot/struct/check/cdc/precheck） |
