...
pub enum Message {
    ...
    AuthenticationGaussDbSha256(AuthenticationGaussDbSha256Body),
    AuthenticationGaussDbMd5Sha256(AuthenticationGaussDbMd5Sha256Body),
    ...
    pub fn parse(buf: &mut BytesMut) -> io::Result<Option<Message>> {
        ...
        let message = match tag {
            ...
            AUTHENTICATION_TAG => match buf.read_i32::<BigEndian>()? {
                ...
                10 => {
                    let storage = buf.read_all();
                    if storage.starts_with(b"SCRAM-SHA-256") {
                        Message::AuthenticationSasl(AuthenticationSaslBody(storage))
                    } else {
                        Message::AuthenticationGaussDbSha256(AuthenticationGaussDbSha256Body(
                            storage,
                        ))
                    }
                }
                11 => {
                    let storage = buf.read_all();
                    if storage.len() == 68 {
                        Message::AuthenticationGaussDbMd5Sha256(AuthenticationGaussDbMd5Sha256Body(
                            storage,
                        ))
                    } else {
...
pub struct AuthenticationGaussDbSha256Body(Bytes);

impl AuthenticationGaussDbSha256Body {
    #[inline]
    pub fn storage_method(&self) -> io::Result<i32> {
        let storage = self.0.as_ref();
        if storage.len() < 4 {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                "invalid gaussdb sha256 auth body length",
            ));
        }
        Ok(BigEndian::read_i32(&storage[..4]))
    }

    #[inline]
    pub fn random64(&self) -> io::Result<&str> {
        read_fixed_str(self.0.as_ref(), 4, 64)
    }

    #[inline]
    pub fn token8(&self) -> io::Result<&str> {
        read_fixed_str(self.0.as_ref(), 68, 8)
    }

    #[inline]
    pub fn iterations(&self) -> io::Result<Option<u32>> {
        let storage = self.0.as_ref();
        if storage.len() < 80 {
            return Ok(None);
        }
        Ok(Some(BigEndian::read_i32(&storage[76..80]) as u32))
    }
}

pub struct AuthenticationGaussDbMd5Sha256Body(Bytes);

impl AuthenticationGaussDbMd5Sha256Body {
    #[inline]
    pub fn random64(&self) -> io::Result<&str> {
        read_fixed_str(self.0.as_ref(), 0, 64)
    }

    #[inline]
    pub fn salt(&self) -> io::Result<[u8; 4]> {
        let storage = self.0.as_ref();
        if storage.len() < 68 {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                "invalid gaussdb md5-sha256 auth body length",
            ));
        }
        let mut salt = [0; 4];
        salt.copy_from_slice(&storage[64..68]);
        Ok(salt)
    }
}

fn read_fixed_str(storage: &[u8], offset: usize, len: usize) -> io::Result<&str> {
    if storage.len() < offset + len {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            "invalid gaussdb auth body length",
        ));
    }
