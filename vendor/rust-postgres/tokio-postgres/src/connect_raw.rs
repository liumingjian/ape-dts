...
use postgres_protocol::authentication::{self, gaussdb};
    ...
    T: AsyncRead + AsyncWrite + Unpin,
{
    ...
    frontend::startup_message_with_version(params, config.protocol_version_minor, &mut buf)
    ...
    T: TlsStream + Unpin,
{
    match stream.try_next().await.map_err(Error::io)? {
        ...
        Some(Message::AuthenticationGaussDbSha256(body)) => {
            can_skip_channel_binding(config)?;
            ...
            let pass = config
                ...
                .as_ref()
                .ok_or_else(|| Error::config("password missing".into()))?;
            let method = gaussdb::PasswordStorageMethod::from_i32(
                body.storage_method().map_err(Error::parse)?,
            )
            .ok_or_else(|| Error::authentication("unsupported GaussDB password storage".into()))?;

            let output = authenticate_gaussdb_sha256(user, pass, method, body).await?;
            authenticate_password(stream, &output).await?;
        }
        Some(Message::AuthenticationGaussDbMd5Sha256(body)) => {
            can_skip_channel_binding(config)?;
            ...
            let pass = config
                ...
                .as_ref()
                .ok_or_else(|| Error::config("password missing".into()))?;
            let output = gaussdb::md5_sha256_password(
                pass,
                body.random64().map_err(Error::parse)?,
                &body.salt().map_err(Error::parse)?,
            );
            authenticate_password(stream, &output).await?;
        }
...
async fn authenticate_gaussdb_sha256(
    user: &str,
    password: &[u8],
    method: gaussdb::PasswordStorageMethod,
    body: postgres_protocol::message::backend::AuthenticationGaussDbSha256Body,
) -> Result<Vec<u8>, Error> {
    match method {
        gaussdb::PasswordStorageMethod::Plain
        | gaussdb::PasswordStorageMethod::Sha256
        | gaussdb::PasswordStorageMethod::Sha256Rfc => Ok(gaussdb::gaussdb_sha256_password(
            password,
            body.random64().map_err(Error::parse)?,
            Some(body.token8().map_err(Error::parse)?),
            body.iterations().map_err(Error::parse)?.unwrap_or(2048),
        )),
        gaussdb::PasswordStorageMethod::Md5 => {
            let salt = stream_sha256_md5_salt(body)?;
            Ok(gaussdb::sha256_md5_password(user, password, &salt))
        }
    }
}

fn stream_sha256_md5_salt(
    body: postgres_protocol::message::backend::AuthenticationGaussDbSha256Body,
) -> Result<[u8; 4], Error> {
    let token = body.token8().map_err(Error::parse)?;
    if token.len() != 8 {
        return Err(Error::authentication(
            "invalid GaussDB SHA256 MD5 salt length".into(),
        ));
    }
    let mut salt = [0; 4];
    for idx in 0..4 {
        salt[idx] = u8::from_str_radix(&token[idx * 2..idx * 2 + 2], 16)
