...
pub(crate) enum PasswordStorageMethod {
    Plain,
    Md5,
    Sha256,
    Sha256Rfc,
...
pub(crate) fn sha256_md5_password(user: &str, password: &str, salt: &[u8; 4]) -> Vec<u8> {
    let mut md5 = Md5::new();
...
pub(crate) fn md5_sha256_password(password: &str, random64: &str, salt: &[u8; 4]) -> Vec<u8> {
    let derived = gaussdb_sha256_password(password, random64, None, DEFAULT_ITERATIONS);
    let source = format!(
        "{}{}{}",
        random64,
        hex_hmac(password.as_bytes(), SERVER_KEY),
        sha256_hex(&derived)
    );
...
pub(crate) fn gaussdb_sha256_password(
    password: &str,
    random64: &str,
    token8: Option<&str>,
    iterations: u32,
    ...
    let key = pbkdf2_hmac_sha1(password.as_bytes(), &salt, iterations.max(1), 32);
