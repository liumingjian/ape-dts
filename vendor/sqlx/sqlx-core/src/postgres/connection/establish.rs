...
use crate::io::Decode;
use crate::postgres::connection::gaussdb::{self, PasswordStorageMethod};
        ...
        loop {
            ...
            match message.format {
                MessageFormat::Authentication => match message.decode()? {
                    ...
                    Authentication::GaussDbSha256(body) => {
                        let method = PasswordStorageMethod::from_u32(body.storage_method()?)
                            .ok_or_else(|| err_protocol!("unsupported GaussDB password storage"))?;
                        let password = options.password.as_deref().unwrap_or_default();
                        let output = match method {
                            PasswordStorageMethod::Plain
                            | PasswordStorageMethod::Sha256
                            | PasswordStorageMethod::Sha256Rfc => gaussdb::gaussdb_sha256_password(
                                password,
                                body.random64()?,
                                Some(body.token8()?),
                                body.iterations().unwrap_or(2048),
                            ),
                            PasswordStorageMethod::Md5 => gaussdb::sha256_md5_password(
                                &options.username,
                                password,
                                &gaussdb_md5_salt(body.token8()?)?,
                            ),
                        };
                        stream.send(Password::Raw(&output)).await?;
                    }

                    Authentication::GaussDbMd5Sha256(body) => {
...
fn gaussdb_md5_salt(token: &str) -> Result<[u8; 4], Error> {
    if token.len() != 8 {
        return Err(err_protocol!("invalid GaussDB SHA256 MD5 salt length"));
    }
    let mut salt = [0_u8; 4];
    for idx in 0..4 {
        salt[idx] =
            u8::from_str_radix(&token[idx * 2..idx * 2 + 2], 16).map_err(Error::protocol)?;
    }
    Ok(salt)
}
